...
def _build_sample_ids_from_df(self, df_clean: pd.DataFrame):
    """
    Επιστρέφει λίστα sample_ids για df_clean.
    Προτιμά έτοιμη στήλη 'sample_id'· αλλιώς φτιάχνει 'S_{participant_id}_{sample_index}'.
    """
    ...
    if not req.issubset(df_clean.columns):
        raise RuntimeError(
            "Cannot build sample_ids: need either 'sample_id' or both 'participant_id' and 'sample_index'.")
    return ["S_"+str(pid)+"_"+str(int(idx)) for pid, idx in zip(df_clean['participant_id'], df_clean['sample_index'])]
...
class RealisticAnalysis:
    ...
    def load_and_prepare_data(self):
        ...
        best_features, best_set_name = self.select_best_clinical_set(
            df, clinical_sets, train_indices)
    ...
    def create_neurogait_kg_embeddings(self, train_participants, test_participants, *args, align_with_kg=False, **kwargs):
        ...
        if align_with_kg:
            ...
            if missing_train or missing_test:
                ...
                if len(available_train) < 10 or len(available_test) < 5:
                    raise ValueError(
                        f"CRITICAL ERROR: Insufficient participant overlap. Available train: {len(available_train)}, test: {len(available_test)}")

                print(
                    f"   📊 Using participant intersection: Train {len(available_train)}/{len(an_train)}, Test {len(available_test)}/{len(an_test)}")
        ...
        try:
            with self._get_neo4j_session() as session:
                ...
                if not pids_train:
                    raise ValueError(
                        "CRITICAL ERROR: No training participant IDs provided")

                train_records = list(session.run(
                    cypher, split="train", pids=pids_train))
                if not train_records:
                    raise ValueError(
                        f"CRITICAL ERROR: No training embeddings found for participants: {pids_train}")
                ...
                for rec in train_records:
                    ...
                    if vec is None:
                        raise ValueError(
                            f"CRITICAL ERROR: No valid embedding vector found for training participant {rec['pid']}")

                    try:
                        X_train.append(np.asarray(vec, dtype=float))
                    except Exception as e:
                        raise ValueError(
                            f"CRITICAL ERROR: Cannot convert embedding vector for training participant {rec['pid']}: {e}")

                # TEST
                pids_test = list(map(str, test_participants))
                if not pids_test:
                    raise ValueError(
                        "CRITICAL ERROR: No test participant IDs provided")

                test_records = list(session.run(
                    cypher, split="test", pids=pids_test))
                if not test_records:
                    raise ValueError(
                        f"CRITICAL ERROR: No test embeddings found for participants: {pids_test}")
                ...
                for rec in test_records:
                    ...
                    if vec is None:
                        raise ValueError(
                            f"CRITICAL ERROR: No valid embedding vector found for test participant {rec['pid']}")

                    try:
                        X_test.append(np.asarray(vec, dtype=float))
                    except Exception as e:
                        raise ValueError(
                            f"CRITICAL ERROR: Cannot convert embedding vector for test participant {rec['pid']}: {e}")
        ...
        if len(X_train) == 0:
            raise ValueError(
                "CRITICAL ERROR: No training embeddings retrieved")
        ...
        if len(set(train_dims)) > 1:
            raise ValueError(
                f"CRITICAL ERROR: Inconsistent training embedding dimensions: {set(train_dims)}")

        if len(set(test_dims)) > 1:
            raise ValueError(
                f"CRITICAL ERROR: Inconsistent test embedding dimensions: {set(test_dims)}")

        if train_dims[0] != test_dims[0]:
            raise ValueError(
                f"CRITICAL ERROR: Dimension mismatch between train ({train_dims[0]}) and test ({test_dims[0]})")
        ...
        if np.isinf(X_train_kg).any() or np.isinf(X_test_kg).any():
            raise ValueError(
                "CRITICAL ERROR: KG embeddings contain infinite values")

        _log(
            "info", f"🧠 KG embeddings ready: Train {X_train_kg.shape}, Test {X_test_kg.shape}")
        return X_train_kg, X_test_kg
    ...
    def statistical_comparison_analysis(self, tier1_results: dict):
        """
        STRICT paired statistics με ευθυγράμμιση σε ΚΟΙΝΑ sample_ids.
        - Απαιτούνται 'ids' σε κάθε best-model entry (βλ. train_optimized_models).
        - Αν δεν υπάρχει τομή IDs -> σφάλμα.
        """
        print("\n📊 DETAILED STATISTICAL ANALYSIS (sample-level, paired):")
        print("="*70)

        # Επιλογή best model ανά προσέγγιση
        best = {}
        for approach_name, models in tier1_results.items():
            if not models:
                continue
            best_auc, best_entry = -1.0, None
            for model_name, metrics in models.items():
                if not all(k in metrics for k in ('proba_test', 'y_test', 'auc', 'ids')):
                    continue
            ...
            if best_entry is not None:
                best[approach_name] = {
                    'ids': np.asarray(best_entry['ids'], dtype=str),
                    'y':   np.asarray(best_entry['y_test']),
                    'p':   np.asarray(best_entry['proba_test'])
                }

        if len(best) < 2:
            raise RuntimeError(
                "Not enough valid approaches with ids/y/p to run paired statistics.")

        # Ζεύγη προσεγγίσεων
        approaches = list(best.keys())
        statistical_results, p_values, comparisons = {}, [], []

        def _align_by_ids(a, b):
            ids_a, y_a, p_a = best[a]['ids'], best[a]['y'], best[a]['p']
            ids_b, y_b, p_b = best[b]['ids'], best[b]['y'], best[b]['p']
            common = sorted(set(ids_a) & set(ids_b))
            if not common:
                raise RuntimeError(
                    f"No common test sample_ids between {a} and {b}.")
            pos_a = {sid: i for i, sid in enumerate(ids_a)}
            pos_b = {sid: i for i, sid in enumerate(ids_b)}
            ia = np.array([pos_a[sid] for sid in common], dtype=int)
            ib = np.array([pos_b[sid] for sid in common], dtype=int)
            y1, p1 = y_a[ia], p_a[ia]
            y2, p2 = y_b[ib], p_b[ib]
            # Ελέγχουμε ότι τα labels είναι ίδια μετά την ευθυγράμμιση
            if not np.array_equal(y1, y2):
                raise RuntimeError(
                    f"Label mismatch after alignment for {a} vs {b}.")
            return np.asarray(common, dtype=str), y1, p1, p2

        # Υποστηρικτικές συναρτήσεις (όπως ήδη έχεις στον κώδικα σου)
        def wilcoxon_rank_biserial_from_trueprob(y_true, p_a, p_b):
            # Wilcoxon σε ζεύγη με μετατροπή σε “wins” βάσει p
            from scipy.stats import wilcoxon
            d = p_a - p_b
            stat, p_val = wilcoxon(
                d, zero_method='wilcox', alternative='two-sided', correction=False, mode='approx')
            # rank-biserial ~ (W+ - W-) / (W+ + W-) -> για signed diffs προσεγγιστικά:
            wins = np.sum(d > 0)
            losses = np.sum(d < 0)
            rbc = 0.0
            total = wins + losses
            if total > 0:
                rbc = (wins - losses) / total
            return stat, p_val, float(rbc)

        def paired_bootstrap_metric_diff(y, p_a, p_b, metric_fn, n_boot=5000, seed=123, threshold=0.5):
            rng = np.random.default_rng(seed)
            n = len(y)
            diffs = []
            for _ in range(n_boot):
                idx = rng.integers(0, n, n)
                yy, pa, pb = y[idx], p_a[idx], p_b[idx]
                if metric_fn is roc_auc_score:
                    ma = roc_auc_score(yy, pa)
                    mb = roc_auc_score(yy, pb)
                elif metric_fn is accuracy_score:
                    ma = accuracy_score(yy, (pa >= threshold).astype(int))
                    mb = accuracy_score(yy, (pb >= threshold).astype(int))
                elif metric_fn is f1_score:
                    ma = f1_score(yy, (pa >= threshold).astype(int))
                    mb = f1_score(yy, (pb >= threshold).astype(int))
                else:
                    raise RuntimeError("Unsupported metric for bootstrap.")
                diffs.append(ma - mb)
            diffs = np.asarray(diffs)
            return float(np.mean(diffs)), (float(np.percentile(diffs, 2.5)), float(np.percentile(diffs, 97.5))), diffs
        ...
        # Τρέχουμε όλα τα ζεύγη με STRICT alignment
        for i in range(len(approaches)):
            for j in range(i+1, len(approaches)):
                a1, a2 = approaches[i], approaches[j]
                common_ids, y, p1, p2 = _align_by_ids(a1, a2)

                print(f"\n🔍 COMPARING (test level): {a1} vs {a2}")
                print(f"   Common paired samples: n={len(common_ids)}")
                ...
                print(
                    f"Wilcoxon: p = {p_val:.4f}, rank-biserial r = {rbc:+.3f} ({rank_biserial_to_label(rbc)})")

                key = f"{a1} vs {a2}"
                statistical_results[key] = {
                    'approach1': a1, 'approach2': a2,
                    'n_common': int(len(common_ids)),
                    'auc_diff': auc_diff, 'auc_ci': auc_ci,
                    'acc_diff': acc_diff, 'acc_ci': acc_ci,
                    'f1_diff': f1_diff,   'f1_ci':  f1_ci,
                    'w_statistic': float(W), 'p_value': float(p_val),
        ...
        if p_values:
            ...
            m = len(p_values)
            adj = np.empty_like(p_values, dtype=float)
            for rank, idx in enumerate(order, start=1):
                adj[idx] = p_values[idx] * m / rank
            adj = np.minimum.accumulate(adj[order[::-1]])[::-1]  # monotonicity
            for key, p_adj in zip(comparisons, adj):
                statistical_results[key]['corrected_p_value'] = float(
                    min(1.0, p_adj))

            # Εκτύπωση σύντομου πίνακα
            print("\n📋 STATISTICAL SUMMARY TABLE (paired bootstrap & Wilcoxon):")
            print("="*110)
            print(
                f"{'Comparison':<35} {'ΔAUC (95% CI)':<28} {'p-value':<10} {'Corrected p':<12} {'r (effect)':<12} {'Sig'}")
            print("="*110)
            for comp in comparisons:
                res = statistical_results[comp]
                ci = res['auc_ci']
                p_raw = res['p_value']
                p_cor = res.get('corrected_p_value', np.nan)
                eff = res['rank_biserial']
                sig = "✅" if p_cor < 0.05 else "📋"
                print(
                    f"{comp:<35} {res['auc_diff']:+.3f} [{ci[0]:.3f},{ci[1]:.3f}]   {p_raw:<10.4f} {p_cor:<12.4f} {eff:+.3f} {sig}")
            print("="*110)
    ...
    def run_enhanced_analysis_with_tuning(self):
        ...
        simple_kg_results = self.train_optimized_models(
            ...
            "Simple KG",
            test_ids=test_ids  # ίδια σειρά/δείγματα με το X_test_scaled
        )
        ...
        if ENHANCED_FEATURES_AVAILABLE:
            ...
            try:
                ...
                enhanced_kg_results = self.train_optimized_models(
                    ...
                    test_ids=test_ids  # ίδια IDs από το ίδιο test_data
                )

            except Exception as e:
                # ΌΧΙ fallback: αν αποτύχει, το αφήνουμε None και συνεχίζουμε αυστηρά
        ...
        # ΌΧΙ fallbacks: απαιτούμε best_config, αλλιώς διακόπτουμε ρητά
        if not best_config:
            raise RuntimeError(
        ...
        tuned_kg_results = self.train_optimized_models(
            ...
            test_ids=test_ids  # ίδια IDs (δεν ρίχνουμε δείγματα εδώ)
        )
